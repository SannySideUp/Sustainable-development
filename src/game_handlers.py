...
This module contains all the game-related command functionality separated from the main CLI class for better organization.
"""
...
class GameHandlers:
    ...
    def handle_load(self, filename: str = None) -> None:
        ...
        self._load_game_file(filename.strip())

    def _load_game_file(self, filename: str) -> None:
        """Load a specific game file and update CLI state."""
        try:
    ...
    def _show_save_files(self) -> None:
        """Show available save files and allow selection."""
        ...
        try:
            ...
            if 0 <= choice < len(save_files):
                self._load_game_file(save_files[choice])
            else:
