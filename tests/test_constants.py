...
class TestConstants:
    ...
    # --- Core Constant Checks ---

    def test_game_core_constants(self):
        """Test fundamental game constants like winning score and default difficulty."""
        # DEFAULT_WINNING_SCORE (Integer)
        assert hasattr(constants, 'DEFAULT_WINNING_SCORE')
        assert isinstance(constants.DEFAULT_WINNING_SCORE, int)

        # DEFAULT_DIFFICULTY (String)
        assert hasattr(constants, 'DEFAULT_DIFFICULTY')
        assert isinstance(constants.DEFAULT_DIFFICULTY, str)
        assert constants.DEFAULT_DIFFICULTY == "casual"
    ...
    def test_default_player_names(self):
        ...
        assert constants.DEFAULT_PLAYER_1_NAME == "Player 1"
        assert constants.DEFAULT_PLAYER_2_NAME == "Player 2"

    # --- State and Header Checks ---

    def test_state_constants(self):
        """Test state constants, including STATE_INIT."""
        states = [
            'STATE_INIT', 'STATE_MENU', 'STATE_PLAYING', 'STATE_SETTINGS',
    ...
    # --- Menu Template Checks (New) ---

    def test_main_menu_templates(self):
        """Test constants related to the main, settings, statistics, and high score menus."""
        menus = {
            'MAIN_MENU': 'Select an option (1-7):',
            'SETTINGS_MENU_TEMPLATE': 'Select an option (1-4):',
            'STATISTICS_MENU': 'View Game History',
            'HIGHSCORES_MENU': 'View Top Scores'
        }
        for const_name, expected_content in menus.items():
            assert hasattr(constants, const_name)
            assert isinstance(getattr(constants, const_name), str)
            assert expected_content in getattr(constants, const_name)

    def test_game_menu_template_content(self):
        """Test the main GAME_MENU_TEMPLATE has expected score fields."""
        assert hasattr(constants, 'GAME_MENU_TEMPLATE')
        template = constants.GAME_MENU_TEMPLATE
        assert 'Player 1' in template
        assert 'Score to Win' in template
        assert 'Select an option' in template

    def test_difficulty_menu_constants(self):
        """Test difficulty menu specific constants."""
        assert hasattr(constants, 'DIFFICULTY_CURRENT_MARKER')
        assert constants.DIFFICULTY_CURRENT_MARKER == " ← CURRENT"

        assert hasattr(constants, 'DIFFICULTY_MENU_TEMPLATE')
        assert 'options_text' in constants.DIFFICULTY_MENU_TEMPLATE

    def test_load_game_menus(self):
        """Test constants for the load game screen."""
        assert hasattr(constants, 'LOAD_GAME_NONE')
        assert 'No save files found.' in constants.LOAD_GAME_NONE

        assert hasattr(constants, 'LOAD_GAME_AVAILABLE')
        assert 'Available saves:' in constants.LOAD_GAME_AVAILABLE
        assert 'save_options' in constants.LOAD_GAME_AVAILABLE # Placeholder check

    def test_player_setup_menu_constants(self):
        """Test constants related to player name setup screens."""
        setup_constants = {
            'SET_P1_NAME_MENU': 'Enter new name for Player 1',
            'SET_P2_NAME_MENU': 'Enter new name for Player 2',
            'P1_NAME_SETUP': '=== ENTER PLAYER 1 NAME ===',
            'P2_NAME_SETUP': '=== ENTER PLAYER 2 NAME ===',
            'PLAYER_SETUP_MENU': 'Enter Player 2\'s name'
        }
        for const_name, expected_content in setup_constants.items():
            assert hasattr(constants, const_name)
            assert isinstance(getattr(constants, const_name), str)
            assert expected_content in getattr(constants, const_name)

    def test_game_rules_constant(self):
        """Test the GAME_RULES block."""
        assert hasattr(constants, 'GAME_RULES')
        assert isinstance(constants.GAME_RULES, str)
        assert "Each turn, a player repeatedly rolls" in constants.GAME_RULES

    # --- Message Checks (Errors, Success, Roll) ---
    ...
    def test_cheat_code_constants(self):
        """Test CHEAT_CODES constant which contains multi-line help."""
        assert hasattr(constants, 'CHEAT_CODES')
        assert isinstance(constants.CHEAT_CODES, str)
        assert "Cheat Code Help" in constants.CHEAT_CODES
        assert "- WIN:" in constants.CHEAT_CODES
    ...
    def test_computer_player_constants(self):
        ...
        assert constants.COMPUTER_PLAYER_NAME == "Computer"
        assert constants.COMPUTER_PLAYER_ID == "computer"
    ...
    def test_cheat_code_help_constant(self):
        """Test CHEAT_CODE_HELP constant (The simple one)."""
    ...
    def test_player_setup_constants(self):
        """Test player setup prompt constants."""
        setup_constants = [
    ...
    def test_settings_menu_constants(self):
        """Test settings menu result messages."""
        settings_constants = [
    ...
    def test_all_constants_are_strings(self):
        """Test that all constants (except the known integer) are non-empty strings."""
        constants_attrs = [
            attr for attr in dir(constants)
            if attr.isupper() and not attr.startswith('_')
        ]

        # Exclude the known integer constant
        if 'DEFAULT_WINNING_SCORE' in constants_attrs:
            constants_attrs.remove('DEFAULT_WINNING_SCORE')

        for attr_name in constants_attrs:
            attr_value = getattr(constants, attr_name)
            assert isinstance(attr_value, str), f"{attr_name} should be a string (found {type(attr_value)})"
            assert len(attr_value) > 0, f"{attr_name} should not be empty"

    def test_format_strings_contain_placeholders(self):
        """Test that format strings contain appropriate placeholders (e.g., '{}' for str.format())."""
        format_strings = [
            constants.INVALID_CHOICE, constants.INVALID_SELECTION, constants.INVALID_INPUT,
            constants.INVALID_DIFFICULTY_CHOICE, constants.UNKNOWN_COMMAND,
            constants.GAME_STARTED_COMPUTER, constants.GAME_STARTED_PLAYER,
            constants.GAME_SAVED, constants.GAME_LOADED, constants.DIFFICULTY_SET,
            constants.PLAYER1_NAME_SET, constants.PLAYER2_NAME_SET,
            constants.ROLLED_MESSAGE, constants.HOLD_MESSAGE, constants.COMPUTER_ROLLED,
            constants.CHEAT_APPLIED, constants.WINNER_DISPLAY, constants.ERROR_SAVING_GAME,
            constants.ERROR_LOADING_GAME, constants.ERROR_STARTING_GAME,
            constants.COMPUTER_TURN_ERROR, constants.ROLL_ERROR, constants.SAVE_FILE_FORMAT,
            constants.CURRENT_NAME_FORMAT, constants.CURRENT_PLAYER_2_FORMAT,
            constants.PLAYER_SCORE_FORMAT, constants.PLAYER2_SCORE_FORMAT,
            constants.CURRENT_PLAYER_FORMAT, constants.TURN_SCORE_FORMAT,
            constants.SCORE_TO_WIN_FORMAT, constants.DIFFICULTY_SET_SUCCESS,
            constants.PLAYER1_NAME_SET_SUCCESS, constants.PLAYER2_NAME_SET_SUCCESS,
            constants.GAME_SAVED_SUCCESS,
            # New template checks for dynamic content
            constants.DIFFICULTY_MENU_TEMPLATE, # uses {options_text} and {max_choice}
            constants.LOAD_GAME_AVAILABLE, # uses {save_options} and {max_choice}
            constants.SET_P1_NAME_MENU, # uses {current_name}
            constants.SET_P2_NAME_MENU, # uses {current_name}
            constants.PLAYER_SETUP_MENU # uses {current_name}
        ]

        # Use {} for simple format checks, or check for named placeholders if the template uses them
        for format_str in format_strings:
            is_valid_placeholder = "{}" in format_str or "{" in format_str and "}" in format_str
            assert is_valid_placeholder, f"Format string should contain placeholders: {format_str}"

    def test_constants_are_immutable(self):
        """
        Test that constants can be modified (standard Python behavior),
        but ensure the test restores the original value.
        """
